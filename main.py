...
class LoginToApp:
    def __init__(self):
        ...
        photo = tk.PhotoImage(file="komparator1.png")
        self.Login.iconphoto(False, photo)
        self.Login.attributes('-topmost', True)
        self.Login.configure(bg='white')

        #self.Login.bind("<Button-1>", self.startMove)
        #self.Login.bind("<ButtonRelease-1>", self.stopMove)      #bindowanie klawiszy dla przesówania okienka
        #self.Login.bind("<B1-Motion>", self.moving)

        myCanvas = tk.Canvas(self.Login, bg='#0052cc', height=57.5, width=480)  # Budowa tła niebieskie
        ...
        myLabel = tk.Label(self.Login, image=my_img)
        myLabel.place(x=10, y=10, height=96, width=96)
        '''
        ...
        self.LoginTitle.place(height=55, width=480, x=-65, y=0)
        ...
        self.UserLoginLabel.bind("<Button-1>", self.forgotpassword)
        ...
        self.CreateNewAccount.bind("<Button-1>", self.createnewaccount)
        ...
        TypeUsers = ["Default", "Admin"]

        self.UserLoginType = ttk.Combobox(self.Login, values=TypeUsers)
    ...
    def createnewaccount(self,x):
        ...
        self.NewUserRequest = tk.Button(self.CNAW,text='Request for Account',font = 14, bg='#0052cc', fg='white',command =self.requestForNewAccount)
    ...
    def forgotpassword(self,x):
        ...
        center_x = int(screen_width / 2 - Login_width / 2)
        center_y = int(screen_height / 2 - Login_height / 2)
        self.CNFW.geometry(f'{Login_width}x{Login_height}+{center_x}+{center_y}')
        self.CNFW.resizable(False, False)
        self.Login.attributes('-topmost')
